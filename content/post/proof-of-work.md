...
So, the question was: How do we "charge" email senders without money? Is there a more equitable way to charge Internet users...using something that they already have? Something like...their computers? And here was Hashcash's ingenuity: make the email senders "spend" their CPU processing time. To "pay" for an email with Hashcash, the sender will first need to solve a puzzle, or a cost function, with their computer.
...
If the email header fails the proof-of-work checks, the recipient stores the email in a separate spam folder or discards it altogether.
