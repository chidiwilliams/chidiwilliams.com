...
The third property looks a bit more daunting than the first two. Remember the example where the correct answer was “STARS”, and we guessed “FRERE”? The first “R” was wrong, while the other “R” was correct. And so, if the result shows a letter is wrong, it doesn’t mean the letter doesn’t occur in the correct answer _at all_. But that it doesn’t occur any _more_ times after it was correct or misplaced.
...
Finally, we can put it all together. At first, we’ll get a list of all possible five-letter words. While the game hasn’t ended, we’ll get the best guess from the list, play the guess, and then update the word list based on the result.
...
{{<iframefigure src="https://chidiwilliams.github.io/wordle" caption="Play against the AI. Try to guess the word and then click 'Show AI Board' to see what the AI played." height="750px" >}}
