...
Let's take a slightly different expression to illustrate this further. This time, we'll put the brackets around the first two numbers in the expression to become: `(3 + 2) * 4 - 1`.

To evaluate this infix expression, we would:

1. Add 3 and 2 to get 5
2. Multiply 5 (from step 1) to 4 to get 20
3. Subtract 1 from 20 (from step 2) to get 19

Again, here we followed the precedence rules of brackets first, then multiplication, then subtraction.

The RPN equivalent of this expression is `3 2 + 4 * 1 -`. And again, you may have picked up that the operators are in the correct evaluation order from left to right: first addition, then multiplication, then subtraction.

We'll evaluate this RPN as before: moving from left to right, we'll find the first operator with two available operands. So we'll add 3 and 2 together first.
...
And finally, we'll subtract 1 from 20 to get 19.

```jsx
20 1 -
```

We can now see that RPN expressions have the same result as their infix equivalents. Take a few moments to study the table below to see how RPN expresses the order of operations using the position of the operators instead of parentheses.
...
In the RPN evaluation process we just discussed, we went back to the start of the expression after each step to read from left to right again. We can use a stack instead to store the values we are not yet ready to evaluate.
...
Popping the last two elements from the stack (1 and 20), subtract the first from the second, and pushing the result (19) back on to the stack:
...
Let's consider a few examples, starting with the infix expression `3 * 4 + 1`. The RPN equivalent of the expression is `3 4 * 1 +`.
...
The next token is the operator `*`. We can't yet tell whether the multiplication operation should be done at this point in the expression (though peeking at the complete infix expression, we know it very well should). So we'll push the operator to `operators` till we know for sure.
...
If the operators already in `operators` have an equal or higher precedence than the next operator, then they should be positioned in the final expression before the next operator. We would first pop them from `operators` and push them on to `out` (we call this process _unwinding_ `operators` on to `out`). And then push the next operator to `operators` as before.
...
To evaluate this expression, we evaluate the section enclosed in parentheses first. The parentheses act like an operator with precedence higher than all the others. The complete evaluation order would be:
...
The RPN of the expression is `3 4 2 - * 1 5 * +`.
